...
const opencodeHooksPluginPath = path.join(repoRoot, '.opencode', 'plugin-support', 'ecc-hooks.ts');
...
test('.opencode/plugin-support/ecc-hooks.ts active plugin banner matches package.json', () => {
  ...
  assert.ok(match, 'Expected .opencode/plugin-support/ecc-hooks.ts to declare an active plugin banner');
  assert.strictEqual(match[1], expectedVersion);
});
